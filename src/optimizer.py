...
def _veredicto(train: dict, test: dict, min_trades_test: int) -> str:
    """Etiqueta de robustez leyendo AMBOS tramos (train y test)."""
    if test["ops"] < min_trades_test:
        return "⚪ pocas ops"
    rentable_ambos = train["profit_factor"] > 1.0 and test["profit_factor"] > 1.0
    if not rentable_ambos:
        return "🔴 no robusta"
    if test["profit_factor"] >= 1.3 and test["ops"] >= min_trades_test:
        return "🟢 robusta"
    return "🟡 dudosa"
...
def explorar_par(
    ...
    for params in combos:
        ...
        filas.append({
            ...
            "retorno_test_pct": me["retorno_pct"], "aciertos_test": me["aciertos"],
...
def _clave_orden(fila: dict) -> tuple:
    """Ordena: primero robustas, luego por profit factor de test (acotado)."""
    prioridad = {"🟢 robusta": 0, "🟡 dudosa": 1, "🔴 no robusta": 2, "⚪ pocas ops": 3}
    pf = fila["pf_test"]
    pf_cmp = 999.0 if pf == float("inf") else pf
    return (prioridad.get(fila["veredicto"], 9), -pf_cmp, -fila["retorno_test_pct"])
